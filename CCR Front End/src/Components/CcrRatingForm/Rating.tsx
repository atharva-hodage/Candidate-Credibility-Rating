...
const Rating: React.FC = () => {
  ...
  const [categories, setCategories] = useState<{ categoryId: number; categoryName: string, categoryWeightage: number }[]>([]);
  const [categoryInputs, setCategoryInputs] = useState<{ [key: number]: string }>({});
  const [triggerEffect, setTriggerEffect] = useState(false);
  const [editingRow, setEditingRow] = useState<number | null>(null);

  const [updatedCategoryWeightages, setUpdatedCategoryWeightages] = useState<{ [key: number]: number }>({});
  

  const updateCategoryWeightage = (categoryId: number, newWeightage: number) => {
  ...
  const updateQuestionWeightage = (questionId:number, newWeightage:number) => {
    setCate1((prevCate1) =>
  ...
  const handleSaveAllCategoryWeightages = () => {
   ...
   console.log(updatedCategoryWeightages)
    ...
    const categoryUpdates = Object.entries(updatedCategoryWeightages).map(([categoryId, categoryWeightage]) => ({
      categoryId: parseInt(categoryId), 
      categoryWeightage: categoryWeightage,
    }));
    
    // Send the array of category updates to the server
    axios.put('http://localhost:8080/updateCategories', categoryUpdates, {
      headers: {
      ...
      .then(() => {
        ...
        Object.entries(updatedCategoryWeightages).forEach(([categoryId, newWeightage]) => {
      ...
      .catch((error) => {
        console.log(error);
        alert("Failed to update one or more category weightages. Keep the total of weightages as 100");
      });
  }    
  ...
  const handleCategoryInputChange = (categoryId: number, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/getAllCategories`).then(
      (response: AxiosResponse<{ categoryId: number; categoryName: string; categoryWeightage: number }[]>) => {
    ...
    axios.get(`http://localhost:8080/getAllQuestion`).then(
  ...
  const addQuestionToCategory = (categoryId: number, categoryName: string) => {
    axios
      .post(`http://localhost:8080/${categoryId}/addQuestion?questionContent=${categoryInputs[categoryId]}`)
      .then(
        (response) => {
  ...
  const handleDeleteCategory = (categoryId: number) => {
    ...
    if (categoryToDelete) {
      if (categoryToDelete.categoryWeightage === 0) {
        ...
        alert("Category weightage must be 0 before deletion.");
      }
    }
  };
  ...
  const handleDeleteQuestion = (questionId: number, categoryId: number) => {
    if (window.confirm("Are you sure you want to delete this question?")) {
      ...
      axios
        .delete(`http://localhost:8080/removeQuestion?questionId=${questionId}`)
        .then(
          (response) => {
            if (response.status === 200) {
              setCate1((prevCate1) =>
 ...
 const handleEditQuestion = (questionId: number) => {
  ...
  const handleEditInputChange = (
    ...
    const updatedQuestions = cate1.map((question) =>
      question.questionId === questionId
        ? { ...question, [field]: field === 'weightage' ? parseFloat(e.target.value) : e.target.value }
        : question
    );
    setCate1(updatedQuestions);
  };
  
  const handleEditWeightageChange = (rowIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSaveEdit = (questionId: number) => {
    ...
    const editedQuestion = cate1.find((question) => question.questionId === questionId);
    ...
    if (editedQuestion) {
      ...
      const totalWeightage = existingQuestions
        .map((q) => (q.questionId === questionId ? updatedWeightage : q.weightage))
      ...
      if (totalWeightage <= categoryWeightage) {
        axios
          .put(
            `http://localhost:8080/updateQuestion?questionId=${editedQuestion.questionId}&questionContent=${editedQuestion.questionContent}&weightage=${updatedWeightage}`
          )
          .then(
            (response) => {
    ...
    <>
      ...
      <div className="container">
        ...
        <form onSubmit={addCategory}>
          ...
          <button type="submit"><b>Add Category</b></button>
        ...
        {categories.map((category, categoryIndex) => (
          <div key={categoryIndex}>
            ...
            <h4 className="category-header">Category : {category.categoryName}
        &nbsp;
        ...
        <>
          <input
            ...
            onChange={(e) =>
              handleCategoryWeightageChange(category.categoryId, e.target.value)
            }
          />
          &nbsp;&nbsp; &nbsp;&nbsp;
        </>
      ...
      <button  className="editCategory" onClick={() => handleEditCategory(category.categoryId)}>
       <b>Edit</b> 
    ...
    <tr key={questionIndex}>
      <td>
        {editingRow === filteredQuestion.questionId ? (
          <input
            ...
            onChange={(e) => handleEditInputChange(filteredQuestion.questionId, e, 'questionContent')}
          />
        ) : (
      ...
      <td>
        {editingRow === filteredQuestion.questionId ? (
          <input
            ...
            onChange={(e) => handleEditInputChange(filteredQuestion.questionId, e, 'weightage')}
          />
        ) : (
      ...
      <td>
        {editingRow === filteredQuestion.questionId ? (
          <>
            <button
              style={{ color: 'green', background: 'none', border: '1px solid green', borderRadius: '4px' }}
              onClick={() => handleSaveEdit(filteredQuestion.questionId)}
            >
              <FontAwesomeIcon icon={faSave} />
            </button>&nbsp;&nbsp;&nbsp;&nbsp;
            <button
              style={{ color: 'blue', background: 'none', border: '1px solid blue', borderRadius: '4px' }}
          ...
          <>
            <button
              style={{ color: 'green', background: 'none', border: '1px solid green', borderRadius: '4px' }}
              onClick={() => handleEditQuestion(filteredQuestion.questionId)}
            >
              <FontAwesomeIcon icon={faEdit} />
            </button>&nbsp;&nbsp;&nbsp;&nbsp;
            <button
              style={{ color: 'red', background: 'none', border: '1px solid red', borderRadius: '4px' }}
              onClick={() => handleDeleteQuestion(filteredQuestion.questionId, category.categoryId)}
            >
              <FontAwesomeIcon icon={faTrash} />
            </button>
          </>
  ...
  <>
            <input className="addQuestion"
              ...
              onChange={(e) => handleCategoryInputChange(category.categoryId, e)}
            />
            
            <button onClick={() => addQuestionToCategory(category.categoryId, category.categoryName)}>
              Add Question
            </button>
       ...
       <br/>
        ...
        <button style={{textAlign:'center'}} onClick={handleSaveAllCategoryWeightages}>Save Category Weightages</button>
      </div>
      <>
    </>
    </>
